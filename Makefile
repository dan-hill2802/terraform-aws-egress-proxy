...
.PHONY: test
test: ## Build, test, and destroy vitals scenario with Kitchen Terraform
	@make setup
	@./run-kitchen.sh --pipeline-id ${PIPELINE_ID} --action test --args "--destroy=always"
	@make delete-test-dir

.PHONY: build
build: ## Build vitals scenario with Kitchen Terraform
	@make setup
	@./run-kitchen.sh --pipeline-id ${PIPELINE_ID} --action converge

.PHONY: verify
verify: ## Verify vitals scenario with Kitchen Terraform
	@make setup
...
.PHONY: destroy
destroy: ## Destroy vitals scenario with Kitchen Terraform
	@./run-kitchen.sh --pipeline-id ${PIPELINE_ID} --action destroy
	@make teardown

.PHONY: debug
debug: ## Debug vitals scenario with Kitchen Terraform
	@make setup
...
delete-test-dir:
	@printf "\nDeleting test/integration/vitals_${PIPELINE_ID}\n"
	@rm -rf test/integration/vitals-${PIPELINE_ID}
